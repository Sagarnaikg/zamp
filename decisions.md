...
**Extension is documented, not automated.** The README carries a step-by-step guide for adding a fourth provider. It stays a code change on purpose — every provider needs its own LangChain integration package, so a plugin system would add indirection without removing the actual work. The design does the next best thing: `PROVIDERS` and `PROVIDER_KEY_VARS` are both `Record<Provider, …>`, so extending the `Provider` union makes the build fail until every required piece is supplied. Verified by doing it — adding a fourth member to the union produces exactly two errors, naming the two files that need entries, and nothing else.
